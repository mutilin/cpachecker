...
public class ThreadState implements AbstractState, AbstractStateWithLocations, Partitionable,
  ...
  public class ThreadStateBuilder {
    ...
    public void addToThreadSet(ThreadLabel label) throws CPATransferException{
      for (ThreadLabel created : tSet) {
        if (created.getName().equals(label.getName()) && created.getVarName().equals(label.getVarName())) {
          //Not supported yet
          throw new CPATransferException("Can not create thread " + label.getName() + ", it was already created");
        }
      }
    ...
    public boolean removeFromThreadSet(ThreadLabel label) {
      ...
      ThreadLabel toRemove = null;
      for (ThreadLabel tmpLabel : tSet) {
        if (tmpLabel.equals(label)) {
          toRemove = tmpLabel;
          break;
        } else if (tmpLabel.getVarName().equals(label.getVarName()) &&
                tmpLabel.getStatus().equals(label.getStatus())) {
          //knows only var name
          toRemove = tmpLabel;
          break;
        } else if (tmpLabel.getName().equals(label.getName()) &&
            tmpLabel.isSelfParallel()) {
