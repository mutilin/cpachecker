...
public abstract class AbstractBAMCache<E> implements BAMCache<E> {
  ...
  public AbstractBAMCache(
      Configuration config,
      Reducer reducer,
      LogManager logger) throws InvalidConfigurationException {
    config.inject(this, AbstractBAMCache.class);
  ...
  public BAMCacheEntry put(
    ...
    BAMCacheEntry entry = getEntry(rs);
    // assert !preciseReachedCache.containsKey(hash);
    preciseReachedCache.put(hash, entry);
    return entry;
  }

  protected abstract BAMCacheEntry getEntry(E element);
  ...
  @Override
  public abstract Collection<ReachedSet> getAllCachedReachedStates();
  ...
  class AbstractStateHash {
    ...
    public boolean equals(Object pObj) {
      if (!(pObj instanceof AbstractBAMCache.AbstractStateHash)) {
        return false;
      }
      if (pObj == this) {
        return true;
